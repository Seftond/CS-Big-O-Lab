...
// ANSWER: 
//In the first one, the letters array is already declared so it is O(1). Then the word that is passed in is 
//parsed into an array. That functionality is O(n) because we don't know how large the word is.
//In the second one, the string is replaced, so that doesn't take any space. But, since the object is created
//dynamically in this example, that operation has a space complexity of O(n).
